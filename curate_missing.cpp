...
std::vector<std::string> get_missing_OR_present(std::vector<std::string> detected_libs, std::unordered_map<std::string, std::unordered_map<std::string, std::string>>json_map, std::string distro_name, std::string flag){
	...
	if(flag == "-p"){
		...
		for(int i=0; i<present_list.size(); i++) std::cout << std::setw(15) << "" << present_list[i] << "(" << detected_libs[i] << ")" << std::endl;
		return present_list;
	}
	...
	if(flag == "-m"){
		...
		for(int i=0; i<missing_list.size(), i++) std::cout << std::setw(15) << "" << missing_list[i] << "("  << detected_libs[i] << ")" << std::endl;
		return missing_list;
	}
}